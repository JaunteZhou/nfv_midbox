...
def insert_function(db, cursor, fe_id, image_id, host_id, func_local_id, ip, pwd, cpu, ram, type, disk, ref_count):
    ...
    # 插入数据
    sql = "INSERT INTO t_function (fe_id, image_id, host_id, func_local_id, ip, pwd, cpu, ram, type, disk, ref_count) " + \
          "VALUES ('%d', '%d', '%d', '%s', '%s', '%s', '%d', '%d', '%d', '%d', '%d')" % \
...
def insert_image(db, cursor, image_id, image_local_id, func, type):
    ...
    func = str(func)
    # 插入数据
    sql = "INSERT INTO t_image (image_id, image_local_id, func, type) " + \
          "VALUES ('%d', '%s', '%s', '%d')" % \
...
def insert_host(db, cursor, host_id, host_name, ip, pwd, cpu, ram, disk):
    ...
    disk = int(disk)
    # 插入数据
    sql = "INSERT INTO t_host (host_id, host_name, ip, pwd, cpu, ram, disk) " + \
...
def insert_flow(db, cursor, flow_id, chain, match_field):
    ...
    # 插入数据
    sql = "INSERT INTO t_flow (flow_id, chain, match_field) " + \
          "VALUES ('%d', '%s', '%s')" % \
...
m_table_items = {
    "t_function": ["fe_id", "image_id", "host_id", "func_local_id", "ip", "pwd", "cpu", "ram", "type", "disk",
                   "ref_count"],
